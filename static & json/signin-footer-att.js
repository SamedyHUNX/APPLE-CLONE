'use strict';

const pContentArray = [
    {0: ['The Apple Online Store users industry-standard encryption to protect the confidentiality of the information you submit. Learn more about our <a href="#">Security Policy</a>.',
         'We approximate your location from your internet IP address by matching it to a geographic region or from the location entered during your previous visit to Apple.']},
    {1: [`<p id="p1"> More ways to shop: 
         <a href="/findstore/findstore.html">Find an Apple Store</a> or 
         <a href="#">other retailer</a> near you. Or call <a href="#">1-800-MY-APPLE</a>.
         </p>`, "United States"]},
    {2: [`Copyright <i class="ri-copyright-line"></i> 2024 Apple Inc. All rights reserved.`,
         `<a id="first-a" href="#">Privacy Policy</a>
          <a class="mid-element" href="#">Terms of Use</a>
          <a class="mid-element" href="#">Sales and Refunds</a>
          <a class="mid-element" href="#">Legal</a>
          <a href="#">Site Map</a>`]}
  ];